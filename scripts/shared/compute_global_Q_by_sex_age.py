import numpy as np
import pandas as pd
from pathlib import Path
from scipy import stats 


ROOT = Path(r"C:/Users/eliza/CPSC_599_CONNECTOMICS/TERMProject")

CONN_DIR    = ROOT / "data/connectomes/cpac/nofilt_noglobal/cc200_z"
MODULE_FILE = ROOT / "results/group_connectomes/CC200_modules.txt"

FEMALE_META = ROOT / "data/female/female_metadata_included.csv"
MALE_META   = ROOT / "data/male/male_metadata_included.csv"

OUT_DIR = ROOT / "results/hubs"
OUT_DIR.mkdir(exist_ok=True, parents=True)

DENSITY = 0.10  # same as your hub script
AGE_GROUPS = ["child", "teen", "young_adult"]  #focus on these


#Load fixed module partition 
mods = pd.read_csv(MODULE_FILE, sep=r"\s+")
mods = mods.sort_values("ROI_index")

roi2mod = mods["Module"].to_numpy()  #length 200, values 1..7
N_ROI   = len(roi2mod)
N_MOD   = int(roi2mod.max())
print(f"Loaded module file with {N_ROI} ROIs, {N_MOD} modules")


#helpers
def threshold_top_density(mat: np.ndarray, density: float = 0.10) -> np.ndarray:
    """Keep top |density| of absolute edges, symmetrize, zero diagonal."""
    if mat.shape[0] != mat.shape[1]:
        raise ValueError(f"Matrix not square: {mat.shape}")
    n = mat.shape[0]

    mat = mat.copy()
    np.fill_diagonal(mat, 0.0)

    iu, ju = np.triu_indices(n, k=1)
    vals = np.abs(mat[iu, ju])

    m = len(vals)
    k = int(np.floor(density * m))
    if k < 1:
        return np.zeros_like(mat)

    thresh = np.partition(vals, -k)[-k]
    mask   = np.abs(mat) >= thresh

    out = np.where(mask, mat, 0.0)
    out = np.triu(out, 1)
    out = out + out.T
    np.fill_diagonal(out, 0.0)
    return out


def add_age_group_column(df: pd.DataFrame) -> pd.DataFrame:
    bins   = [0, 13, 18, 30, 45, 120]
    labels = ["child", "teen", "young_adult", "adult", "older"]
    df = df.copy()
    df["AGE_GROUP"] = pd.cut(df["AGE_AT_SCAN"], bins=bins,
                             labels=labels, right=False)
    return df


#modularity Q with fixed partition
def compute_modularity_fixed(W: np.ndarray, roi2mod: np.ndarray) -> float:
    if W.shape[0] != W.shape[1]:
        raise ValueError(f"W not square: {W.shape}")
    if W.shape[0] != len(roi2mod):
        raise ValueError("W and roi2mod length mismatch")

    #ensure symmetry, zero diagonal
    W = 0.5 * (W + W.T)
    np.fill_diagonal(W, 0.0)

    k = W.sum(axis=1) #strength per node
    m = k.sum() / 2.0 #total edge weight

    if m == 0:
        return 0.0

    #community indicator matrix
    comm = roi2mod.astype(int)
    same_comm = (comm[:, None] == comm[None, :]).astype(float)

    #modularity matrix term 
    expected = np.outer(k, k) / (2.0 * m)
    B = W - expected

    Q = (B * same_comm).sum() / (2.0 * m)
    return float(Q)

#main
def process_sex_for_Q(sex_label: str, meta_path: Path):
    print(f"\n=== {sex_label.upper()} ===")
    meta = pd.read_csv(meta_path)
    meta.columns = meta.columns.str.strip()
    meta = add_age_group_column(meta)

    required = {"FILE_ID", "DX_GROUP", "AGE_AT_SCAN", "func_mean_fd", "AGE_GROUP"}
    missing  = required - set(meta.columns)
    if missing:
        raise ValueError(f"{meta_path} missing columns: {missing}")

    rows = []

    for age_group in AGE_GROUPS:
        sub = meta[meta["AGE_GROUP"] == age_group].copy()
        if sub.empty:
            print(f"[{sex_label} {age_group}] no subjects, skipping")
            continue

        print(f"[{sex_label} {age_group}] N = {len(sub)}")
        print(sub["DX_GROUP"].value_counts().rename({1: "ASD", 2: "CTL"}))

        n_bad = 0

        for _, row in sub.iterrows():
            fid     = row["FILE_ID"]
            conn_fp = CONN_DIR / f"{fid}.npy"

            if not conn_fp.exists():
                print(f"  [WARN] missing connectome: {conn_fp}")
                n_bad += 1
                continue

            mat = np.load(conn_fp)
            if mat.shape != (N_ROI, N_ROI):
                print(f"  [WARN] skipping {fid}: shape {mat.shape}")
                n_bad += 1
                continue

            W = threshold_top_density(mat, density=DENSITY)
            Q = compute_modularity_fixed(W, roi2mod)

            rows.append(
                {
                    "FILE_ID": fid,
                    "sex": sex_label,
                    "AGE_GROUP": age_group,
                    "DX_GROUP": row["DX_GROUP"],
                    "AGE_AT_SCAN": row["AGE_AT_SCAN"],
                    "func_mean_fd": row["func_mean_fd"],
                    "Q": Q,
                }
            )

        print(f"[{sex_label} {age_group}] usable subjects: {len(sub) - n_bad}, skipped: {n_bad}")

    if not rows:
        print(f"No usable subjects for {sex_label}")
        return None

    out_df = pd.DataFrame(rows)
    out_path = OUT_DIR / f"{sex_label}_global_Q_by_age.csv"
    out_df.to_csv(out_path, index=False)
    print(f"Saved {out_path} with {len(out_df)} rows")


    print("\nASD vs CTL Q effects:")
    for age_group in AGE_GROUPS:
        sg = out_df[out_df["AGE_GROUP"] == age_group]
        if sg.empty:
            continue
        asd = sg[sg["DX_GROUP"] == 1]["Q"]
        ctl = sg[sg["DX_GROUP"] == 2]["Q"]
        if len(asd) < 3 or len(ctl) < 3:
            print(f"  {sex_label} {age_group}: not enough subjects for t-test")
            continue
        t, p = stats.ttest_ind(asd, ctl, equal_var=False)
        print(
            f"  {sex_label} {age_group}: "
            f"mean Q ASD={asd.mean():.3f}, CTL={ctl.mean():.3f}, "
            f"Δ(CTL-ASD)={ctl.mean()-asd.mean():.3f}, p={p:.3f}"
        )

    return out_df


if __name__ == "__main__":
    female_Q = process_sex_for_Q("female", FEMALE_META)
    male_Q   = process_sex_for_Q("male",   MALE_META)

    print("\nDone.")
