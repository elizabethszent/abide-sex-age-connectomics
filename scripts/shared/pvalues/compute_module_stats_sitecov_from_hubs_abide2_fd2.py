# scripts/shared/pvalues/compute_module_stats_sitecov_from_hubs_abide2_fd2.py
...
# ABIDE2 SITE map should come from your combined metadata (recommended)
META_COMBINED = ROOT / "data" / "metadata" / "ABIDE2_phenotypes_combined.csv"

SCENARIO = "ABIDE2_fd2"

# Hub files produced by your ABIDE2 hubs script
HUB_GLOB = "*_pc_z_strengthPosNeg_abide2_fd2.csv"
...
def build_site_map_from_combined(meta_csv: Path) -> dict[int, str]:
    """
    Build map: SUB_ID (int) -> SITE_ID (str) from ABIDE2 combined metadata CSV.
...
def parse_sex_age_from_filename(fp: Path) -> tuple[str, str] | None:
    ...
    """
    m = re.match(
        r"^(female|male)_(child_0_9|preteen_10_12|teen_13_17|adult_18_plus)_pc_z_strengthPosNeg_abide2_fd2\.csv$",
        fp.name,
        flags=re.IGNORECASE,
    )
    if not m:
...
def ensure_required_columns(df: pd.DataFrame, fp: Path):
    # subject_int is preferred; but we can derive it from FILE_ID if needed
    need = {"DX_GROUP", "module", "node"}
    missing = need - set(df.columns)
    if missing:
        raise RuntimeError(f"{fp} missing required columns: {missing}. Found: {sorted(df.columns)}")

    if ("subject_int" not in df.columns) and ("FILE_ID" not in df.columns):
        raise RuntimeError(f"{fp} must contain either subject_int or FILE_ID to map subjects.")
