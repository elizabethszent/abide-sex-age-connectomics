...
# CC200 base node file (coords + labels)
TEMPLATE_NODE = BASE / r"results\vis\brainnet\CC200_base.node"

# Louvain modules (1..K, ideally K=7) – same for all groups
MODULES_FILE = BASE / r"results\group_connectomes\CC200_modules.npy"

# Group-mean connectivity matrices (200 x 200, Z-mean)
GROUP_MATS = {
...
# Edge density for Gephi (top X% strongest positive edges per group)
...
# === LOAD MODULES (FROM LOUVAIN, 7 MODULES) ===
...
def compute_node_strength(A: np.ndarray) -> np.ndarray:
    """
    Weighted degree (strength): sum of positive weights for each node.
    A is symmetric [n, n].
    """
...
def write_gephi_files_for_group(name: str, A: np.ndarray):
    """
    Write nodes and edges CSV for Gephi for a single group.
    Nodes: Id,Label,x,y,z,Module,Strength
    Edges: Source,Target,Type,Weight
    """
    # 1) Prepare adjacency: positive weights only, no self-loops
    M = np.maximum(A, 0.0)
    np.fill_diagonal(M, 0.0)

    # 2) Node strength for this group
