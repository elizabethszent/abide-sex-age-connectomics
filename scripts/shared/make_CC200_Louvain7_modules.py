# scripts/shared/make_CC200_Louvain7_modules.py
import numpy as np
import pandas as pd
...
# ==== CONFIG ====
# 1) Path to a 200x200 group-mean connectivity matrix (Fisher-z or r)
#    For now, point to your CHILD grand-mean matrix (all children, ASD+CTL, both sexes)
GRAND_MEAN_PATH = Path(
    "C:/Users/eliza/CPSC_599_CONNECTOMICS/TERMProject/results/shared/child_grand_mean_connectome.npy"
)

# 2) Output CSV with ROI -> module labels (1..7)
OUT_CSV = Path(
    "C:/Users/eliza/CPSC_599_CONNECTOMICS/TERMProject/data/parcellation/CC200_Louvain7_modules.csv"
)

# 3) Optionally: path to your BrainNet CC200_base.node to sanity-check N=200
BRAINNET_NODE = Path(
    "C:/Users/eliza/CPSC_599_CONNECTOMICS/TERMProject/data/parcellation/CC200_base.node"
)
...
def load_cc200_node_count(node_path: Path) -> int:
    """Count lines in the .node file (each line = one ROI)."""
...
def build_graph_from_matrix(C: np.ndarray, threshold: float = None) -> nx.Graph:
    """
    Build a weighted, undirected graph from a 2D connectivity matrix.
    If threshold is not None, we only keep edges with |w| >= threshold.
    """
    ...
    # If matrix is Fisher-z, you *could* convert back to r, but community
    # detection is scale-invariant enough that you can use Z directly.
    W = C.copy()
    ...
    if threshold is not None:
        ...
        # keep all edges (but we’ll still ignore exact zeros)
        mask = np.abs(W) > 0
...
def louvain_with_k_approx(G: nx.Graph, k_target: int, n_trials: int = 50, random_state_base: int = 42):
    """
    Run Louvain multiple times with different seeds and pick the partition
    whose number of communities is closest to k_target.
    Returns: best_partition (dict: node -> community_id), best_k
    """
    best_part = None
    best_k = None
    best_diff = None
...
def relabel_communities_to_1based(partition: dict) -> np.ndarray:
    """
    Take a dict {node: community_label} where community_label might be
    arbitrary ints, and map them to 1..K with sorted order.
    Returns an array of shape [n_nodes] where index i is module label for ROI i+1.
    """
...
def main():
    ...
    # 1) Load grand-mean 200x200 matrix
    ...
    # 3) Louvain, searching for ~7 modules
    ...
    # 4) Relabel modules to 1..K and build ROI->module table
