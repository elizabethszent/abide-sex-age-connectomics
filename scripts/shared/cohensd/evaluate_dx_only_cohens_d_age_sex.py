...
OUT_DIR = ROOT / "results" / "qc" / "cohens_d_dx_only_age_sex"
...
def build_subject_level_values(df: pd.DataFrame, age_group: str, sex_code: int, metric: str, module: int) -> pd.DataFrame:
...
def summarize_dx_groups(subj: pd.DataFrame):
    ctl = subj.loc[subj["DX_GROUP"] == 2, "value"]
    asd = subj.loc[subj["DX_GROUP"] == 1, "value"]

    out = {
        "n_CTL": int(len(ctl)),
        "mean_CTL": safe_float(ctl.mean()) if len(ctl) else np.nan,
        "sd_CTL": safe_float(ctl.std(ddof=1)) if len(ctl) > 1 else np.nan,
        "median_CTL": safe_float(ctl.median()) if len(ctl) else np.nan,
        "min_CTL": safe_float(ctl.min()) if len(ctl) else np.nan,
        "max_CTL": safe_float(ctl.max()) if len(ctl) else np.nan,

        "n_ASD": int(len(asd)),
        "mean_ASD": safe_float(asd.mean()) if len(asd) else np.nan,
        "sd_ASD": safe_float(asd.std(ddof=1)) if len(asd) > 1 else np.nan,
        "median_ASD": safe_float(asd.median()) if len(asd) else np.nan,
        "min_ASD": safe_float(asd.min()) if len(asd) else np.nan,
        "max_ASD": safe_float(asd.max()) if len(asd) else np.nan,
    }

    out["cohens_d_CTL_minus_ASD"] = cohens_d(
        mean_ctl=out["mean_CTL"],
        mean_asd=out["mean_ASD"],
        sd_ctl=out["sd_CTL"],
        sd_asd=out["sd_ASD"],
        n_ctl=out["n_CTL"],
        n_asd=out["n_ASD"],
    )
    out["abs_d"] = abs(out["cohens_d_CTL_minus_ASD"]) if pd.notna(out["cohens_d_CTL_minus_ASD"]) else np.nan
    return out
...
def run():
    ...
    for fp in node_files:
        ...
        for age_group in AGE_GROUPS:
            for sex_code, sex_label in SEX_MAP.items():
                sub_stratum = df[(df["AGE_GROUP"] == age_group) & (df["SEX"] == sex_code)].copy()
                if sub_stratum.empty:
                    continue

                for metric in METRIC_ALIASES.keys():
                    metric_col = resolve_metric_column(sub_stratum, metric)
                    if metric_col is None:
                        continue
                    ...
                    for module in modules_present:
                        ...
                        row.update(summarize_dx_groups(subj))
        ...
        if not scenario_df.empty:
            ...
            scenario_path = scenario_dir / "dx_only_cohens_d_age_sex.csv"
            scenario_df.to_csv(scenario_path, index=False)

            ranked_path = scenario_dir / "dx_only_cohens_d_age_sex__ranked_by_abs_d.csv"
            scenario_df.sort_values("abs_d", ascending=False).to_csv(ranked_path, index=False)
    ...
    overall_path = OUT_DIR / "dx_only_cohens_d_age_sex__all_scenarios.csv"
    out_df.to_csv(overall_path, index=False)

    overall_ranked = OUT_DIR / "dx_only_cohens_d_age_sex__all_scenarios__ranked_by_abs_d.csv"
    out_df.sort_values("abs_d", ascending=False).to_csv(overall_ranked, index=False)
    ...
    print("\n[TOP 30 by |d|]")
    print(
        out_df.sort_values("abs_d", ascending=False)[show_cols]
        .head(30)
