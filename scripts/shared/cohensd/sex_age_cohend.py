...
OUT_DIR = Path("cohens_d_module_matrices_fd0p3")
...
def cohens_d_matrix(ctl_stack: np.ndarray, asd_stack: np.ndarray) -> np.ndarray:
    n1 = ctl_stack.shape[0]
    n2 = asd_stack.shape[0]

    mean1 = np.nanmean(ctl_stack, axis=0)
    mean2 = np.nanmean(asd_stack, axis=0)

    sd1 = np.nanstd(ctl_stack, axis=0, ddof=1)
    sd2 = np.nanstd(asd_stack, axis=0, ddof=1)

    pooled_var = ((n1 - 1) * (sd1 ** 2) + (n2 - 1) * (sd2 ** 2)) / max(n1 + n2 - 2, 1)
    pooled_sd = np.sqrt(pooled_var)

    d = np.divide(
        mean1 - mean2,
        pooled_sd,
        out=np.zeros_like(mean1, dtype=float),
...
def plot_age_pair(
    age_label: str,
    female_d: Optional[np.ndarray],
    male_d: Optional[np.ndarray],
    ...
    panels = [
        ("Female", female_d, female_n, axes[0]),
        ("Male", male_d, male_n, axes[1]),
    ]

    for sex_label, dmat, (n_ctl, n_asd), ax in panels:
        if dmat is None:
            ax.axis("off")
        ...
        cbar.set_label("Cohen's d (CTL - ASD)")
...
def plot_all_age_groups(
    ...
    d_mats: dict[tuple[str, str], np.ndarray] = {}
    ...
    for age_label in AGE_ORDER:
        for sex_label in ["Female", "Male"]:
            ...
            d = cohens_d_matrix(ctl_stack, asd_stack)
            d_mats[key] = d
            all_abs.append(np.nanmax(np.abs(d)))

    if not all_abs:
        raise RuntimeError(
            "No valid age/sex groups had enough subjects to compute Cohen's d.\n"
    ...
    for age_label in AGE_ORDER:
        female_d = d_mats.get((age_label, "Female"))
        male_d = d_mats.get((age_label, "Male"))
        female_n = counts.get((age_label, "Female"), (0, 0))
        male_n = counts.get((age_label, "Male"), (0, 0))

        if female_d is None and male_d is None:
            continue

        out_path = OUT_DIR / f"cohens_d_{age_label.lower()}.png"
        plot_age_pair(
            age_label=age_label,
            female_d=female_d,
            male_d=male_d,
