...
# --- CONFIG ---
PHENO_PATH = r"data\phenotypes\Phenotypic_V1_0b_preprocessed1.csv" 
OUT_DIR    = r"data\roi_timeseries"
PIPELINE   = "cpac"
...
def load_file_ids(pheno_csv):
    ...
    with open(pheno_csv, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if fid and sex == "2":# only females
                ids.append(fid)
    return ids

if __name__ == "__main__":
    file_ids = load_file_ids(PHENO_PATH)
    print(f"Found {len(file_ids)} female FILE_IDs")
