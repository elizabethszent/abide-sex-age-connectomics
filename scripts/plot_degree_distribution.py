...
deg_files = sorted(glob.glob("results/degrees/*.csv"))
if not deg_files:
...
meta_path = Path("results/metrics_merged.csv")
if not meta_path.exists():
    raise FileNotFoundError("results/metrics_merged.csv not found. Re-run your metrics merge step.")
...
plt.tight_layout()
plt.savefig("results/degree_distribution_groups.png", dpi=200)
plt.show()
