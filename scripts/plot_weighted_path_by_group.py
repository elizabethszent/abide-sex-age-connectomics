...
if "group" not in W.columns:
    Mgrp = pd.read_csv("results/metrics_merged.csv")[["FILE_ID", "DX_GROUP"]]
    Mgrp["group"] = Mgrp["DX_GROUP"].map({1: "ASD", 2: "Control"})
    W = W.merge(Mgrp[["FILE_ID", "group"]], on="FILE_ID", how="left")

#bring in covariates
Mcov = pd.read_csv("results/metrics_merged.csv")
...
plt.tight_layout()
plt.savefig("results/weighted_path_groups.png", dpi=200)
plt.close()
print("Saved: results/weighted_path_groups.png")
