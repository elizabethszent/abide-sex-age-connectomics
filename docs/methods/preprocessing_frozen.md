...
- confounds JSON sidecar when available
- CC200 atlas image
...
## Denoising model
Primary model: 32P-style no-GSR nuisance model plus cosine drift regressors.
...
Sensitivity model:
- 36P-style model with global signal regression (GSR), plus cosine drift regressors
...
ROI time series are extracted for each CC200 parcel using the mean voxelwise signal within each parcel. Subsequent nuisance regression, temporal processing, and censoring are applied according to the frozen pipeline implementation in the canonical preprocessing script.
...
3. regenerate manifests and QC summaries
